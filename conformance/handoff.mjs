...
export function registerHandoffContractTests(createHandoff) {
  ...
  test('两个客户端持有同一版本时，后写入不会覆盖先写入的结果或产生成功回执', () => {
    ...
    const beforeConflict = handoff.read();
    ...
    assert.equal(handoff.readReceipts().length, 1);
  });
  ...
  test('调用者修改读取快照或返回回执，不能绕过写入口改变共享状态', () => {
    ...
    receipts[0].actor = '改名';
    receipts.push({});
    assert.equal(handoff.read().revision, 2);
    assert.equal(handoff.read().summary, 'A 的进度');
    assert.equal(handoff.readReceipts().length, 1);
    assert.equal(handoff.readReceipts()[0].actor, 'A');
    assert.equal(handoff.readReceipts()[0].afterRevision, 2);
  });

  test('无效版本或空进展不会推进状态', () => {
    const handoff = fresh();
    for (const revision of [undefined, 0, -1, 1.2, '1', NaN, Infinity, Number.MAX_SAFE_INTEGER + 1]) {
      assert.throws(() => write(handoff, revision), TypeError);
    }
    for (const field of ['actor', 'summary', 'nextStep']) {
      assert.throws(() => handoff.write({ actor: 'A', expectedRevision: 1, summary: '进度', nextStep: '继续', [field]: ' ' }), TypeError);
    }
    assert.equal(handoff.read().revision, 1);
