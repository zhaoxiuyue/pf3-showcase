...
[English](README.md) · **[讨论你的协作流程](COLLABORATE.md)** · [设计说明](docs/design.zh-CN.md) · [运行示例](#几分钟跑通交接)

我是 **Elara（Xiuyue Zhao）**，PF3 的设计与建设者，以个人身份提供 **AI 协作流程设计、MCP 与工具接入、定制原型开发**。

## 从作品到合作

适合带来的问题：换 AI 窗口后反复解释、现有工具接不上、结果散落难以接手、工作完成后难以核验。

从一个**范围明确的付费验证**开始：围绕一个流程交付可运行的原型，并约定验收条件。先描述最近一次卡点，双方判断是否匹配，再确认书面范围、报价和排期后动工。

**[讨论你的协作流程](COLLABORATE.md)** · **[XiuyueZhao@outlook.com](mailto:XiuyueZhao@outlook.com)**
...
MountainRS（山地遥感物理基座）是作者的遥感研究项目。它的 PF3 树保留了两条失败的数据入口路线：过于复杂的 GEE 预筛，以及当前数据窗中没有任何单景能满足的覆盖要求。缩小 ROI 的调试路线则保持暂停，并写明恢复条件。新窗口先读到这些结论，再判断接下来该试什么。
...
这个 Showcase 还包含可运行的**交接协议草案、JSON Schema、独立参考实现与契约测试**。示例通过乐观版本校验，展示其中一项性质：旧窗口的写回无法静默覆盖更新后的共享状态。
...
展示包版本为 **0.2.8**，协议为 **PF3 Handoff v0.1 draft**。Schema 本轮未改动，`$id` 仍固定到 `v0.2.1`；[协议文档](protocol/README.zh-CN.md#版本与示例)说明单条消息与交接轨迹的区别。
